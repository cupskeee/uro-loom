...
| **World Dashboard** | per-world overview — branches (heads), active vs ended campaigns, markers, thread/faction counts | multiple | 🟡 partial today; richer with 🔴 branch endpoint |
| **Timeline / Branch Graph** | the commit DAG as a git-style graph (depth, parent, forks, markers) | `commits` + `branches` + `markers` | 🔴 lineage/branches endpoints |
| **Event-Log Inspector** | filterable stream over events by branch/commit, `event_type`, `entity_ref`, `caused_by` (player/module/external/…) | `events` | 🔴 events read endpoint |
| **Commit / Beat Detail** | one commit's ordered events, parent + hash, the `BeatResolved` narration | `commits` + `events` | 🔴 |
| **Actor / NPC Directory & Profile** | branch-scoped actors (tier T0–T3, status alive/dead, aliases); profile drills into the opaque sheet | `proj_actors` + `proj_sheets` | 🟡 via `state`; richer 🔴 |
| **Epistemic Explorer** | claims (truth true/false/unknown, origin, `created_day`) + a belief fan-out graph (confidence, `learned_from`) | `proj_claims` + `proj_beliefs` | 🔴 |
| **Faction & Relationship Graph** | node-link graph of factions + typed edges (`at_war_with`, `member_of`, `rules`, `located_in`) | `proj_factions` + `proj_edges` | 🔴 |
...
| **Counters Dashboard** | engine-owned integer counters (tension/heat/influence…) by scope + trajectory | `proj_counters` | 🔴 |
| **Memory / Recall Inspector** | `memory_index` rows per branch + a "what would recall surface here?" probe | `memory_index` | 🔴 |
...
| **Preview Beat (dry-run)** | events an intent *would* commit, no write | preview | 🔴 dry-run endpoint |

## Operate (lifecycle writes → events/session)

| Surface | Does | Backend |
|---------|------|---------|
| **New / Import World** | blank world, or import a pack (validate → optional backfill → seed) | 🟡/🔴 |
| **Fork / Branch Operations** | fork from any commit/marker (adopt-PC, new seed, time-skip); add markers | 🔴 |
| **Campaign Lifecycle** | new (adopt/fresh PC), join, end (marker) | 🟢 new/join · 🔴 end |
...
| **Time-Skip** | advance in-fiction time + fire downtime agenda rules | 🟢 `POST …/time-skip` |
| **Codex / Participant Memory** | add/list fork-surviving out-of-world notes (pinned/refs) | 🔴 |
| **World Authoring aids** | validate / backfill / probe / export / import | 🔴 |
...
| **Ruleset Viewer** | registered rulesets (`id@version`, e.g. `uro-basic` d20 / `uro-pbta` 2d6) + each sheet shape | 🔴 (registry read endpoint) |
| **Reaction-Layer / Rule-Pack Viewer** | a pack's declarative `rules.yaml`/`agendas.yaml` (conditions + closed action set) + module-caused beats | 🔴 |
| **Ops / Usage & Telemetry** | `llm_calls` telemetry (provider/model, tokens, cost, latency) + probe quality signals | 🔴 `GET /usage` (deferred) — **the dashboard/billing UX is explicitly Loom's job**, fed by engine metering |
